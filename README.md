...
> 基于 **Spring AI Alibaba** + **通义千问** 构建的全功能 QQ 机器人，集成 ReAct Agent、三层记忆、RAG 知识库、图片生成、定时提醒、Skill 扩展体系、**OpenCLI 网站数据采集**等能力。
...
8. [配置说明](#8-配置说明)
9. [工具列表](#9-工具列表)
10. [OpenCLI 网站数据采集](#10-opencli-网站数据采集)
11. [Skill 扩展体系](#11-skill-扩展体系)
12. [记忆体系](#12-记忆体系)
...
支持CLI控制：
<div style="display:flex;gap:8px;">
  <img src="./image/4.jpg" width="32%">
  <img src="./image/5.jpg" width="32%">
  <img src="./image/6.jpg" width="32%">
</div>
...
集成 [OpenCLI](https://github.com/jackwener/OpenCLI) 后，AI Agent 还能直接抓取 B站热榜、知乎热搜、小红书、微博、Reddit、HackerNews 等 87+ 网站的实时数据，无需用户手动查询。
...
| **OpenCLI 集成** | 接入 [OpenCLI](https://github.com/jackwener/OpenCLI) 87+ 网站适配器，B站/知乎/小红书/微博/Reddit 等热榜数据一句话获取 |
...
| 技术 | 用途 |
|---|---|
| **@jackwener/opencli（npm）** | 87+ 网站 CLI 适配器，公开接口直接调用，cookie 模式复用浏览器会话 |
| **OpenCliTool.java** | 封装 `ProcessBuilder` 调用 opencli，注入 AgentTool 体系，自动注册 |
...
| Node.js | 18+ | Skill 市场 CLI + **OpenCLI**（可选） |
...
INFO  ReactAgentService - 已自动注册 20 个工具: [WeatherTool, OpenCliTool, ReminderTool, ...]
...
| `listOpenCliCommands()` | "opencli 支持哪些网站" | 调用 `opencli list` 返回全部适配器列表 |
| `runOpenCliCommand(cmd)` | "B站今天热门是什么" → `bilibili hot --limit 10` | ProcessBuilder 调用 opencli，输出限 3000 字 |
| `getOpenCliSiteHelp(site)` | "知乎有哪些命令" | 调用 `opencli zhihu --help` 返回子命令说明 |
...
[OpenCLI](https://github.com/jackwener/OpenCLI) 是一个开源 CLI 框架，将 87+ 网站（B站、知乎、小红书、微博、Reddit、HackerNews 等）封装为标准化命令行接口，支持两种运行模式：

| 模式 | 说明 | 典型站点 |
|---|---|---|
| **public（公开接口）** | 直接调用网站公开 API，无需登录，零配置 | HackerNews、Reddit、36氪、arxiv |
| **cookie（浏览器会话）** | 复用已登录的 Chrome 会话，需安装 Browser Bridge 扩展 | B站、知乎、小红书、微博、Twitter |

### 集成方式

项目通过 `OpenCliTool.java` 将 opencli 接入 Agent 工具体系：

```
用户消息："B站今天什么视频最火？"
    │
    ▼
ReactAgent 推理 → 调用 runOpenCliCommand("bilibili hot --limit 10")
    │
    ▼
OpenCliTool.runOpenCli()
    │
    ├── ProcessBuilder 执行: /opt/homebrew/bin/opencli bilibili hot --limit 10
    ├── 读取 stdout（限 3000 字）
    └── 返回结构化文本给 Agent → Agent 整理回复 → 发送给用户
```
...
opencli arxiv search --query "LLM agent"
```

**Cookie 模式命令（需安装 Browser Bridge 扩展）：**

1. 从 [Releases](https://github.com/jackwener/opencli/releases) 下载 `opencli-extension.zip`
2. Chrome → `chrome://extensions` → 开启开发者模式 → 加载已解压的扩展程序
3. 打开目标网站并确保已登录，然后运行：

```bash
opencli bilibili hot --limit 10
opencli zhihu hot --limit 10
opencli xiaohongshu search --query "旅行"
...
在 QQ 中直接对话，Agent 自动判断何时调用 opencli：

```
你 → B站今天热门视频有哪些？
小小豆 → （调用 runOpenCliCommand("bilibili hot --limit 10")）
         1. 标题：xxx  作者：xxx  播放：xxx万
         2. ...

你 → HackerNews 今天最热的帖子
小小豆 → （调用 runOpenCliCommand("hackernews top --limit 5")）
         1. 标题：...  分数：1200  评论：300
         ...

你 → opencli 支持哪些网站？
小小豆 → （调用 listOpenCliCommands()）
         支持的站点：bilibili、zhihu、xiaohongshu、weibo、reddit、hackernews...
```
...
| 分类 | 站点 |
|---|---|
| 国内资讯 | 知乎、微博、小红书、B站、36氪、虎扑、V2EX、豆瓣 |
| 电商 | 淘宝、京东、1688、亚马逊、闲鱼 |
| 海外社区 | Reddit、HackerNews、ProductHunt、Medium、Lobsters |
| 学术/技术 | arxiv、GitHub（通过 gh）、Stack Overflow |
| 社交媒体 | Twitter/X、Instagram、Bluesky、微信、微信读书 |
| 财经 | 雪球、新浪财经、Bloomberg、Yahoo Finance |
| 桌面应用 | Cursor、ChatGPT、Notion、Antigravity（Electron CDP） |

> 完整列表执行 `opencli list` 查看，或参考 [OpenCLI 官方文档](https://github.com/jackwener/OpenCLI)。
...
**Q：OpenCLI 命令返回"opencli 未安装或未找到"？**

执行 `npm install -g @jackwener/opencli` 安装，安装后执行 `opencli --version` 验证。如仍报错，检查 `/opt/homebrew/bin/opencli` 或 `/usr/local/bin/opencli` 是否存在。

**Q：B站/知乎等 cookie 模式命令返回空数据或"Browser Bridge not connected"？**

cookie 模式需要安装 OpenCLI 浏览器扩展并打开目标网站：
1. 从 [OpenCLI Releases](https://github.com/jackwener/opencli/releases) 下载 `opencli-extension.zip`
2. Chrome → `chrome://extensions` → 开启开发者模式 → 加载已解压的扩展程序
3. 打开并登录 B站/知乎等目标网站后再试

HackerNews、Reddit、36氪 等公开接口命令无需浏览器，可直接使用。
...
│       │   │       ├── OpenCliTool.java           # OpenCLI 网站数据采集（87+ 站点）
